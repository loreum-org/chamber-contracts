...
contract DeployCreate2 is Script {
    ...
    function run() external {
        ...
        bytes memory chamberBeaconByteCode = abi.encodePacked(type(LoreumBeacon).creationCode, abi.encode(address(chamberImpl), msg.sender));
        ...
        console2.log("Chamber LoreumBeacon address: ", chamberBeacon);

        bytes32 registryBeaconSalt = keccak256(abi.encodePacked(vm.envString("REGISTRY_BEACON_SALT")));
        Registry registryImpl = new Registry();
        bytes memory registryBeaconByteCode = abi.encodePacked(type(LoreumBeacon).creationCode, abi.encode(address(registryImpl), msg.sender));
        ...
        console2.log("Registry LoreumBeacon address: ", registryBeacon);
    }
}
